...
def _inject_matlab_path():
    ...
    extern_bin_dir = os.path.join(matlab_root, "extern", "bin", arch)
    engine_native_dir = os.path.join(matlab_engine_dist, "matlab", "engine", arch)

    # matlab/__init__.py refuses to load if it can't find dist/matlab/engine/_arch.txt
    # (generated by pip install, not shipped with MATLAB). Setting MWE_INSTALL makes it
    # take a documented fallback path that adds extern/bin/<arch> to sys.path instead.
    os.environ.setdefault("MWE_INSTALL", matlab_root)

    # Add MATLAB's Python engine dist to sys.path so 'import matlab.engine' resolves
    # to MATLAB's own files. Also add the arch-specific native dirs directly so the
    # pyd extensions (matlabengineforpython_abi3, matlabmultidimarrayforpython) are
    # importable regardless of how matlab/__init__.py bootstraps its paths.
    for path in [matlab_engine_dist, engine_native_dir, extern_bin_dir]:
    ...
    if sys.platform == "win32" and hasattr(os, "add_dll_directory"):
        bin_dir = os.path.join(matlab_root, "bin", arch)
        # Also prepend bin/<arch> to PATH — some MATLAB DLLs load their own dependencies
        # via plain LoadLibrary, which still honors PATH even when add_dll_directory is used.
